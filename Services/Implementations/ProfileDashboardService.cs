		...
		ILogger<ProfileDashboardService> _logger) : IProfileDashboardService
	{
		...
		public async Task<BioSummaryDTO> UpdateBioSummaryAsync(BioSummaryDTO bioSummaryDTO)
		{
			...
			catch (Exception)
			{
				...
				bioSummaryLog.UserIP = _authenticationService.GetUserIP();
				bioSummaryLog.UserName = currentUser.UserName;
				bioSummaryLog.RenderedMessage = "User unauthorized to update profile bio summary.";
				bioSummaryLog.AdditionalData = $"User tried to update profile bio summary that does not belong to them. profile data faculty member id: {personalData.FacultyMemberId}, Logged in user id: {currentUser.UserId}.";
				bioSummaryLog.Timestamp = DateTime.Now;
				_logger.LogWarning("{@LogDetails}", bioSummaryLog);
				#endregion
				throw;
		...
		public async Task<SkillsDTO> UpdateSkillAsync(SkillsDTO skillsDTO)
		{
			...
			catch (Exception)
			{
				...
				skillsLog.UserIP = _authenticationService.GetUserIP();
				skillsLog.UserName = currentUser.UserName;
				skillsLog.RenderedMessage = "User unauthorized to update profile skills.";
				skillsLog.AdditionalData = $"User tried to update profile skill that does not belong to them. profile data faculty member id: {personalData.FacultyMemberId}, Logged in user id: {currentUser.UserId}.";
				skillsLog.Timestamp = DateTime.Now;
				_logger.LogWarning("{@LogDetails}", skillsLog);
				#endregion
				throw;
