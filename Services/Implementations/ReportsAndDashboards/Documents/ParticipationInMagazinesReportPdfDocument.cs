...
{
    public class ParticipationInMagazinesReportPdfDocument : IDocument
    {
        private readonly List<ParticipationInMagazinesReportResponseDTO> _data;
        ...
        public ParticipationInMagazinesReportPdfDocument(
            List<ParticipationInMagazinesReportResponseDTO> data,
            List<Faculty> faculties,
            List<Department> departments,
            string? notes)
        {
            _data = data ?? new List<ParticipationInMagazinesReportResponseDTO>();
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                page.Content().PaddingTop(40).PaddingBottom(50).PaddingHorizontal(40).Column(col =>
                {
                    ...
                    SectionTitle(col, "أولاً: بيانات المشاركة في المجلات العلمية");
                    ComposeParticipationsTable(col);
        ...
        private void ComposeHeader(IContainer c)
        {
            c.PaddingTop(15).PaddingHorizontal(35).Row(row =>
            {
                ...
                row.RelativeItem().AlignCenter().PaddingRight(20).Column(titleCol => {
                    titleCol.Item().Text("تقرير المشاركة في المجلات العلمية").FontSize(16).Bold().FontFamily("Amiri").FontColor(White);
                ...
                row.AutoItem().Width(110).AlignLeft().Column(meta => {
                    ...
                    meta.Item().PaddingTop(4).AlignLeft().Text(t => { t.Span("رقم التقرير: ").FontColor(White).FontSize(9); t.Span($"#MAG-{DateTime.UtcNow:yyyyMMdd}").FontColor(GoldLt).Bold().FontSize(9); });
                });
            });
        }
        ...
        private void ComposeReportIntroduction(ColumnDescriptor col)
        {
            col.Item().PaddingBottom(10).Text(t =>
            {
                ...
                t.Span("هذا التقرير يستعرض إحصائيات مشاركة أعضاء هيئة التدريس في المجلات العلمية (تحكيم/تحرير/نشر) بناءً على النطاق الجغرافي أو التخصصي.").FontColor(Navy).FontSize(10).Bold();
            });
        }

        private void ComposeParticipationsTable(ColumnDescriptor col)
        {
            col.Item().PaddingTop(12).PaddingBottom(20).Border(1).BorderColor(Border).Table(table =>
            {
                ...
                table.Header(h => { HeaderCell(h.Cell(), "نوع المشاركة"); HeaderCell(h.Cell(), "العدد"); });
                ...
                foreach (var item in _data)
                {
                    ...
                    foreach (var part in item.Participations)
                    {
                        var bg = even ? RowAlt : White;
                        BodyCell(table.Cell(), part.ParticipationType, bg, false);
                        BodyCell(table.Cell(), part.NoOfParticipations.ToString(), bg, true);
                        even = !even;
                    }
                }
            });
            ...
            col.Item().PaddingBottom(10).Border(1).BorderColor(Border).Row(row =>
            {
                ...
                row.RelativeItem().Background(NoteYellow).Padding(15).Column(inner =>
                {
                    ...
                    string insights = _data.Any()
                        ? $"تم رصد مشاركات لعدد ({_data.Count}) من أعضاء هيئة التدريس في المجلات العلمية، مما يعزز الحضور الأكاديمي الدولي والمحلي للجامعة."
                        : "لا توجد بيانات مشاركات متاحة للتحليل حالياً.";
