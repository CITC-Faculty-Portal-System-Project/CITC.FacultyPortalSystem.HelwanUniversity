...
{
    internal class ResearchesPerYearReportPdfDocument : IDocument
    {
        private readonly List<Faculty> _faculties;
        private readonly List<Department> _departments;
        private readonly List<ResearchesPerYearReportResponseDTO> _data;
        private readonly List<int> _pubYears;
        ...
        public ResearchesPerYearReportPdfDocument(
            List<Faculty> faculties,
            List<Department> departments,
            List<ResearchesPerYearReportResponseDTO> data,
            List<int> pubYears,
            string? notes)
        {
            _data = data ?? new List<ResearchesPerYearReportResponseDTO>();
            _notes = notes;
            _faculties = faculties ?? new List<Faculty>();
            _departments = departments ?? new List<Department>();
            _pubYears = pubYears ?? new List<int>();
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                page.Content().PaddingTop(30).PaddingBottom(50).PaddingHorizontal(40).Column(col =>
                {
                    ...
                    ComposeYearsWarningBox(col);

                    ComposeReportIntroduction(col);

                    SectionTitle(col, "أولاً: قائمة الإنتاج العلمي (الأبحاث)");
        ...
        private void ComposeHeader(IContainer c)
        {
            c.PaddingTop(15).PaddingHorizontal(35).Row(row =>
            {
                ...
                {
                    titleCol.Item().AlignCenter().Text("تقرير سنوي عن الابحاث العلمية").FontSize(18).Bold().FontColor(Colors.White).FontFamily("Amiri");
                ...
                row.AutoItem().Width(120).AlignMiddle().AlignLeft().Column(meta =>
                {
                    ...
                    meta.Item().PaddingTop(4).AlignLeft().Text(t =>
                    {
                        ...
                        t.Span($"#CU{DateTime.UtcNow:MMdd}").FontColor(GoldLt).Bold().FontSize(9);
                    });
                });
            });
        }

        private void ComposeYearsWarningBox(ColumnDescriptor col)
        {
            string yearsText = _pubYears.Any() ? string.Join("، ", _pubYears.OrderBy(y => y)) : "جميع الأعوام البحثية المتاحة";

            col.Item().PaddingBottom(15).Border(1).BorderColor(Border).Row(row =>
            {
                row.ConstantItem(6).Background(DangerRed);
                row.RelativeItem().Background(NoteYellow).Padding(12).Row(innerRow =>
                {
                    innerRow.RelativeItem().AlignMiddle().AlignRight().Text(t =>
                    {
                        t.Span("⚠️ نطاق الفلترة والسنوات المحددة للتقرير: ").FontColor(DangerRed).Bold().FontSize(10);
                        t.Span(yearsText).FontColor(Text).Bold().FontSize(10);
                    });
                });
            });
        }
        ...
        private void ComposeReportIntroduction(ColumnDescriptor col)
        {
            ...
            string yearsText = _pubYears.Any() ? string.Join("، ", _pubYears.OrderBy(y => y)) : "جميع الأعوام";

            col.Item().PaddingBottom(12).Text(t =>
            {
                t.Span("ملحوظة الكليات: ").FontColor(Gold).FontFamily("Amiri").FontSize(12).Bold();

                if (!hasFaculties && !hasDepartments)
                {
                    t.Span($"هذا التقرير عام وشامل لجميع كليات وأقسام الجامعة عن الفترة البحثية للأعوام ({yearsText}).").FontColor(Navy).FontFamily("Cairo").FontSize(10).Bold();
                }
                else if (hasFaculties && !hasDepartments)
                {
                    ...
                    t.Span($") عن الفترة البحثية للأعوام ({yearsText}) ولم يتم تحديد أقسام علمية معينة.").FontColor(Gold).FontFamily("Amiri").FontSize(12).Italic();
                }
                else
                {
                    var uniqueFacultyNames = string.Join("، ", _faculties!.Select(f => f.NameAR));
                    ...
                    t.Span(uniqueFacultyNames).FontColor(Navy).FontFamily("Cairo").FontSize(10).Bold();
                    t.Span($") للأقسام العلمية المحددة عن الفترة البحثية للأعوام ({yearsText}): ").FontColor(Gold).FontFamily("Amiri").FontSize(12).Italic();
                    ...
                    for (int i = 0; i < _departments!.Count; i++)
                    {
                        ...
                        {
                            _ = t.Span("، ").FontColor(Text).FontFamily("Cairo").FontSize(10);
                        }
                    }
        ...
        private void ComposeFacultyTable(ColumnDescriptor col)
        {
            col.Item().PaddingTop(5).PaddingBottom(14).Table(table =>
            {
                ...
                {
                    HeaderCell(h.Cell(), "#");
                    HeaderCell(h.Cell(), "اسم البحث");
                    HeaderCell(h.Cell(), "نوع النشر");
                    HeaderCell(h.Cell(), "سنة النشر");
                });

                int index = 1;
                bool even = false;

                foreach (var research in _data)
                {
                    var bg = even ? RowAlt : White;

                    string arabicPubType = research.PublicationType switch
                    {
                        PublicationType.Local => "محلي",
                        PublicationType.International => "دولي",
                        PublicationType.Unspecified => "غير محدد",
                        _ => "غير محدد"
                    };

                    BodyCell(table.Cell(), index.ToString(), bg, center: true);
                    BodyCell(table.Cell(), research.ResearchTitle, bg, center: false);
                    BodyCell(table.Cell(), arabicPubType, bg, center: true);
                    BodyCell(table.Cell(), research.PubYear.ToString(), bg, center: true);
        ...
        private void ComposeInsightsBox(ColumnDescriptor col)
        {
            bool hasFaculties = _faculties != null && _faculties.Any();
            bool hasDepartments = _departments != null && _departments.Any();
            string yearsText = _pubYears.Any() ? string.Join("، ", _pubYears.OrderBy(y => y)) : "المحددة";
            ...
            col.Item().PaddingBottom(10).Border(1).BorderColor(Border).Row(row =>
            {
                ...
                row.RelativeItem().Background(NoteYellow).Padding(15).Column(inner =>
                {
                    ...
                    else
                    {
                        ...
                        if (!hasFaculties && !hasDepartments)
                        {
                            insightsText = $"بناءً على إحصائيات الأعوام ({yearsText})، يوضح التحليل الإحصائي العام توزيع وعناوين الإنتاج البحثي على مستوى كافة كليات وأقسام الجامعة للفترة المحددة.";
                        }
                        else if (hasFaculties && !hasDepartments)
                        {
                            var uniqueFacultyNames = string.Join("، ", _faculties!.Select(f => f.NameAR));
                            insightsText = $"بناءً على إحصائيات الأعوام ({yearsText})، يتناول هذا التحليل قياس كفاءة العناوين والأبحاث المنشورة إجمالياً على مستوى كليات ({uniqueFacultyNames}).";
                        }
                        else
                        {
                            var firstDept = _departments!.FirstOrDefault();
                            string targetDept = firstDept != null ? firstDept.NameAR : "القسم المختار";
                            insightsText = $"بناءً على إحصائيات الأعوام ({yearsText})، أظهرت البيانات جودة وتنوعاً ممتازاً في الإنتاجية البحثية المسجلة لعناوين أبحاث قسم {targetDept}.";
                        }
        ...
        private void ComposeUserNotesBox(ColumnDescriptor col)
        {
            col.Item().PaddingTop(12).PaddingBottom(20).Border(1).BorderColor(Border).Row(row =>
            {
                ...
                row.RelativeItem().Background(White).Padding(15).Column(inner =>
                {
                    ...
                    inner.Item().PaddingTop(8).MinHeight(60).Text(_notes ?? "لا توجد ملاحظات إضافية")
        ...
        private static void ComposeSignatures(ColumnDescriptor col)
        {
            col.Item().PaddingTop(25).Row(row =>
            {
                row.RelativeItem().AlignCenter().Column(c =>
                {
                    ...
                    c.Item().PaddingTop(5).AlignCenter().Text("ختم الكلية").FontSize(11).FontColor(Text);
                });
                ...
                {
                    c.Item().Width(180).BorderBottom(1).BorderColor(Border).Height(30);
                    c.Item().PaddingTop(5).AlignCenter().Text("توقيع العميد").FontSize(11).FontColor(Text);
                });
            });
        }
