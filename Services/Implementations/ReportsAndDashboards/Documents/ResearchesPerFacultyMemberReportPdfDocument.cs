...
{
    internal class ResearchesPerFacultyMemberReportPdfDocument : IDocument
    {
        private readonly List<Faculty> _faculties;
        private readonly List<Department> _departments;
        private readonly List<FacultyMembersResearchesReportResponseDTO> _data;
        ...
        public ResearchesPerFacultyMemberReportPdfDocument(
            List<Faculty> faculties,
            List<Department> departments,
            List<FacultyMembersResearchesReportResponseDTO> data,
            List<int> pubYears,
            string? notes)
        {
            _data = data ?? new List<FacultyMembersResearchesReportResponseDTO>();
            _notes = notes;
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                page.Content().PaddingTop(30).PaddingBottom(50).PaddingHorizontal(40).Column(col =>
                {
                    ...
                    SectionTitle(col, "أولاً: بيانات الإنتاج البحثي");
                    ComposeFacultyTable(col);

                    ComposeYearNotice(col);
        ...
        private void ComposeHeader(IContainer c)
        {
            c.PaddingTop(15).PaddingHorizontal(35).Row(row =>
            {
                ...
                {
                    titleCol.Item().AlignCenter().Text("إحصائية أبحاث أعضاء هيئة التدريس").FontSize(18).Bold().FontColor(Colors.White).FontFamily("Amiri");
        ...
        private void ComposeFacultyTable(ColumnDescriptor col)
        {
            col.Item().PaddingTop(5).PaddingBottom(14).Table(table =>
            {
                ...
                table.Header(h =>
                {
                    ...
                    HeaderCell(h.Cell(), "الأبحاث الدولية");
                    HeaderCell(h.Cell(), "الأبحاث المحلية");
                });
                ...
                foreach (var member in _data)
                {
                    ...
                    BodyCell(table.Cell(), member.FacultyMemberName, bg, center: false);
        ...
        private void ComposeYearNotice(ColumnDescriptor col)
        {
            col.Item().PaddingBottom(15).Row(row =>
            {
                row.AutoItem().Text("⚠️ ").FontSize(10).FontColor(DangerRed);
                row.RelativeItem().Text(t =>
                {
                    if (_pubYears == null || !_pubYears.Any())
                    {
                        t.Span("ملاحظة: هذا التقرير مستخرج طبقاً لإحصائيات جميع الأبحاث المسجلة على النظام حتى الآن.").FontSize(10).Bold().FontColor(DangerRed);
                    }
                    else
                    {
                        string yearsString = string.Join("، ", _pubYears.OrderBy(y => y));
                        t.Span("ملاحظة: هذا التقرير مستخرج طبقاً لإحصائيات سنوات ").FontSize(10).Bold().FontColor(DangerRed);
                        t.Span($"({yearsString}) ").FontSize(10).Bold().FontColor(Navy);
                        t.Span("فقط.").FontSize(10).Bold().FontColor(DangerRed);
                    }
                });
            });
        }
            ...
            col.Item().PaddingBottom(10).Border(1).BorderColor(Border).Row(row =>
            {
                ...
                row.RelativeItem().Background(NoteYellow).Padding(15).Column(inner =>
                {
                    ...
                    else
                    {
                        ...
                        if (!hasFaculties && !hasDepartments)
                        {
                            insightsText = "يوضح التحليل الإحصائي العام توزيع نسب الإنتاج البحثي على مستوى كافة كليات وأقسام الجامعة للفترة المحددة.";
                        }
                        else if (hasFaculties && !hasDepartments)
                        {
                            var uniqueFacultyNames = string.Join("، ", _faculties!.Select(f => f.NameAR));
                            insightsText = $"يتناول هذا التحليل قياس كفاءة الإنتاجية البحثية الإجمالية على مستوى كليات ({uniqueFacultyNames}) للفترة المختارة.";
                        }
                        else
                        {
                            var firstDept = _departments!.FirstOrDefault();
                            string targetDept = firstDept != null ? firstDept.NameAR : "القسم المختار";
                            insightsText = $"يتصدر قسم {targetDept} الإنتاج البحثي بنسبة 100.0% من إجمالي أبحاث الكلية للفترة المختارة.";
                        }

                        inner.Item().PaddingTop(8).Text(insightsText).FontSize(11).LineHeight(1.8f).Bold();
                    }
                });
            });
        }
            ...
            col.Item().PaddingTop(12).PaddingBottom(20).Border(1).BorderColor(Border).Row(row =>
            {
                ...
                row.RelativeItem().Background(White).Padding(15).Column(inner =>
                {
                    ...
                    inner.Item().PaddingTop(8).MinHeight(60).Text(_notes ?? "لا توجد ملاحظات إضافية")
                         .FontSize(11).LineHeight(1.8f).FontColor(Muted);
                });
            });
        }
        ...
        private static void ComposeSignatures(ColumnDescriptor col)
        {
            col.Item().PaddingTop(15).Row(row =>
            {
                ...
                {
                    c.Item().Width(180).BorderBottom(1).BorderColor(Border).Height(35);
                    c.Item().PaddingTop(5).AlignCenter().Text("ختم الكلية").FontSize(10).FontColor(Text);
                });
                ...
                {
                    c.Item().Width(180).BorderBottom(1).BorderColor(Border).Height(35);
                    c.Item().PaddingTop(5).AlignCenter().Text("توقيع عميد الكلية").FontSize(10).FontColor(Text);
                });
            });
        }
        ...
        {
            c.PaddingVertical(10).PaddingHorizontal(45).Row(row =>
            {
                ...
                {
                    t.Span("الصفحة ").FontColor(Colors.White).FontSize(9);
                    t.CurrentPageNumber().FontColor(Colors.White).Bold().FontSize(9);
                    t.Span(" من ").FontColor(Colors.White).FontSize(9);
                    t.TotalPages().FontColor(Colors.White).Bold().FontSize(9);
                });
            });
        }
