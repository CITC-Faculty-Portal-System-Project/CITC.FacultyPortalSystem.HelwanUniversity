...
{
    internal class ReviewingArticleReportPDFDocument : IDocument
    {
        private readonly List<Faculty> _faculties;
        private readonly List<Department> _departments;
        private readonly List<ReviewingArticlesReportResponseDTO> _data;
        ...
        public ReviewingArticleReportPDFDocument(
            List<Faculty> faculties,
            List<Department> departments,
            List<ReviewingArticlesReportResponseDTO> data,
            string? notes)
        {
            _data = data ?? new List<ReviewingArticlesReportResponseDTO>();
            _notes = notes;
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                page.Content().PaddingTop(30).PaddingBottom(50).PaddingHorizontal(40).Column(col =>
                {
                    ...
                    SectionTitle(col, "أولاً: قائمة مشاركات التحكيم");
        ...
        private void ComposeHeader(IContainer c)
        {
            c.PaddingTop(15).PaddingHorizontal(35).Row(row =>
            {
                ...
                {
                    titleCol.Item().AlignCenter().Text("تقرير عن تحكيم المقالات العلمية").FontSize(18).Bold().FontColor(Colors.White).FontFamily("Amiri");
        ...
        private void ComposeReportIntroduction(ColumnDescriptor col)
        {
            ...
            col.Item().PaddingBottom(10).Text(t =>
            {
                ...
                t.Span(hasFaculties ? "هذا التقرير مخصص لرصد عمليات تحكيم المقالات العلمية للكليات والأقسام المحددة." : "هذا التقرير عام وشامل لجميع عمليات تحكيم المقالات بكافة كليات وأقسام الجامعة.")
        ...
        private void ComposeDataTable(ColumnDescriptor col)
        {
            ...
            {
                table.ColumnsDefinition(c => { c.ConstantColumn(40); c.RelativeColumn(3); c.RelativeColumn(1); });
                table.Header(h =>
                {
                    HeaderCell(h.Cell(), "#");
                    HeaderCell(h.Cell(), "اسم عضو هيئة التدريس");
                    HeaderCell(h.Cell(), "عدد المقالات");
                });
                int index = 1;
                foreach (var item in _data)
                {
                    ...
                    BodyCell(table.Cell(), item.NoOfArticles.ToString(), bg, true);
                    index++;
                }
            });
        }
        ...
        private void ComposeInsightsBox(ColumnDescriptor col)
        {
            col.Item().PaddingBottom(10).Border(1).BorderColor(Border).Row(row =>
            {
                ...
                row.RelativeItem().Background(NoteYellow).Padding(15).Column(inner =>
                {
                    ...
                    inner.Item().PaddingTop(8).Text(_data.Any() ? $"تم رصد نشاط تحكيمي لعدد ({_data.Count}) من أعضاء هيئة التدريس، مما يعكس المساهمة الفعالة في مراجعة الإنتاج العلمي." : "لا توجد بيانات متاحة للتحليل.").FontSize(11).Bold();
                });
            });
        }
