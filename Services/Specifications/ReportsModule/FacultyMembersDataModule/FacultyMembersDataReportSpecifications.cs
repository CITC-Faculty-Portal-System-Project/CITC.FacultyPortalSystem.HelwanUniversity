            ...
            string? search = null)
        {
            ...
            switch (parameters.Sorting)
            {
                ...
                case FacultyMembersReportSortingOptions.NoOfAwardsAsc:
                    AddOrderBy(fd => fd.PrizesAndRewards.Count(pr => !pr.IsDeleted));
                    break;

                case FacultyMembersReportSortingOptions.NoOfAwardsDesc:
                    AddOrderByDescending(fd => fd.PrizesAndRewards.Count(pr => !pr.IsDeleted));
                    break;
            ...
            AddIncludes(fd => fd.ContactData!);
            AddIncludes(fd => fd.ResearchContributions!);
            AddIncludes(fd => fd.Patents!);
            AddIncludes(fd => fd.PrizesAndRewards!);
        }
